    ...
    a.arrowchat = function () {
        ...
        function buildApplicationButtons() {
            ...
            for (b in apps) {
                if (typeof apps[b][0] != "undefined") {
                    ...
                    if (apps[b][12] == "1" || apps[b][9] != "1" && u_id == "" || u_id != "" && apps[b][9] != "1" && u_is_guest == "1") {
                        ...
                        if (b && b != null) {
                            ...
                            a.each(b, function (e, l) {
                                a.each(l, function (f, h) {
                                    a("<div/>").attr("id", "arrowchat_alert_" + h.alert_id).html(h.markup).appendTo(a("#arrowchat_notifications_content"));
                                    i++;
                                });
                            });
                            if (i == 0) {
        ...
        function toggleUserChatTab(b, c, d, e, l, f) {
            if (G[b] != null) {
                ...
                longname = c.length > 19 ? c.substr(0, 19) + "..." : c;
                ...
                a("#arrowchat_file_upload_" + b).uploadify({
                    uploader: path + "includes/js/uploadify/uploadify.swf",
                    script: path + "includes/classes/class_uploads.php",
                    cancelImg: path + "includes/js/uploadify/cancel.png",
                    hideButton: true,
                    ButtonText: "Browse",
                    wmode: "transparent",
                    scriptData: {
                    ...
                    width: 42,
                    auto: true,
                    fileExt: "*.jpg;*.gif;*.png;*.zip;*.rar;*.jpeg;*.txt;*.doc;*.mp3;*.wmv;*.avi;*.mp4;*.docx;*.wav",
                    fileDesc: "Supported File Types (.JPG, .JPEG, .GIF, .PNG, .WMV, .AVI, .MP4, .ZIP, .RAR, .MP3, .WAV, .TXT, .DOC, .DOCX)",
                    onOpen: function () {
        ...
        function buildBuddyListTab() {
                ...
                a.ajax({
                    ...
                    success: function (b) {
                        if (b && b != null) {
                            ...
                            a("<option/>").attr("value", "0").html("Select").appendTo(a(".arrowchat_block_menu select"));
                            a.each(b, function (e, l) {
                                a.each(l, function (f, h) {
            ...
            $buddy_list_tab.click(function () {
                ...
                if (a(window).height() - $userstab_popup.height() < 70 && a("#arrowchat_userscontent").height() <= a("#arrowchat_userslist_available").height() + a("#arrowchat_userslist_busy").height() + a("#arrowchat_userslist_away").height()) {
                    a("#arrowchat_userscontent").css("height", a(window).height() - 110 + "px");
                } else {
        ...
        function addMessageToChatroom(b, c, d) {
            ...
            if (a("#arrowchat_chatroom_message_" + b).length > 0) {
                ...
                $chatroom_chat.append("<div class=\"arrowchat_chatroom_box_message\" id=\"arrowchat_chatroom_message_" + b + "\"><img class=\"arrowchat_chatroom_message_avatar\" src=\"" + u_avatar + "\" alt=\"\" /><div class=\"arrowchat_chatroom_message_name\">" + c + ":</div><div class=\"arrowchat_chatroom_message_content  arrowchat_self\">" + formatTimestamp(new Date(Math.floor((new Date).getTime() / 1000) * 1000)) + d + "</div></div>");
                $chatroom_chat.scrollTop(50000);
            }
            showChatroomTime();
        }
        function addMessageToChatbox(b, c, d, i, e, l, f) {
            ...
            if (uc_name[b] == null || uc_name[b] == "") {
                ...
                j != b && i != 1 && notifyNewMessage(b, 1, 1);
            }
        }
        ...
        function loadChatroomList() {
            if (chatroomreceived == 0) {
                a("#arrowchat_chatroom_leave").hide();
                a.ajax({
                    url: k + "includes/json/receive/receive_chatroom_list.php",
                    cache: false,
                    data: {
                        chatroom_window: u_chatroom_open,
                        chatroom_stay: u_chatroom_stay
        ...
        function addNotification(alert_id, markup) {
            a("<div/>").attr("id", "arrowchat_alert_" + alert_id).html(markup).appendTo(a("#arrowchat_notifications_content"));
            Q++;
        }
            ...
            if (j == from && uc_name[from] != "" && uc_name[from] != null) {
                ...
                if (a("#arrowchat_message_" + id).length > 0) {
                    ...
                    if (sent - B > 60 || B == null || N != full_name) {
                        a(".arrowchat_tabcontenttext", $user_popups[from]).append("<div class=\"arrowchat_chatboxmessage arrowchat_clearfix\" id=\"arrowchat_message_" + id + "\"><div class=\"arrowchat_chatboxmessagefrom" + f + "\"><div class=\"arrowchat_disable_avatars_name\">" + fromname + "</div><img alt=\"" + tooltip + "\" class=\"arrowchat_chatbox_avatar\" src=\"" + avatar + "\" /></div><div class=\"arrowchat_chatboxmessage_wrapper\"><div class=\"arrowchat_chatboxmessagecontent" + f + "\">" + formatTimestamp(new Date(sent * 1000)) + message + "</div></div></div>");
                        B = sent;
                        N = full_name;
                        last_id[from] = id;
                    } else {
        ...
        function pushReceive(data) {
            ...
            if ("messages" in data) {
                ...
                u_sounds == 1 && playNewMessageSound();
                showTimeAndTooltip();
                K = 1;
                D = E;
            }
        ...
        arguments.callee.getUser = function (b, c, d) {
            a.ajax({
                url: k + "includes/json/receive/receive_user.php",
                data: {
                    userid: b
