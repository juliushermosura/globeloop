    ...
    $(document).ready(function() {
        ...
        $('#show_hide_members').click(function() {
            ...
            if ($('.members_list').is(':visible')) {
                ...
                $(this).text('Show Members (" . count($data['GroupMember']) . ")');
            }
            return false;
        });
        ...
        $('#friend_join_group').click(function() {
            $.ajax({
                url: '/users/friend_join_group',
                data: { user_id: $('#invitation').val(), group_id: '" . $data['Group']['id'] . "' },
                type: 'POST'
            }).done(function(msg) {
                $('#invitation').tokenInput('clear');
            }).fail(function() {
                alert('Oops! Something went wrong during adding friend to this group. You may try again later.');
            });
            return false;
        });
...
<div style="margin-bottom: 10px">
    <div style="margin-bottom: 10px;">
        <?php echo $this->Html->link('Show Members (' . count($data['GroupMember']) . ')', '#show_hide_members', array('escape' => false, 'id' => 'show_hide_members')) ?>
    </div>

    <div class="members_list">
        <?php foreach($data['GroupMember'] as $member): ?>
    ...
    <div class="actions">
        <div style="display: inline-block; vertical-align: top;">Join your friends to this group </div>
        ...
        <div style="display: inline-block; vertical-align: top; line-height: 2.1"><a href="#friend_join_group" id="friend_join_group">Send</a></div>
    </div>
...
<?php echo $this->element('group_posts',array('data'=>$data2, 'parent' => $data['Group']['id'], 'type'=>'Group')) ?> 
